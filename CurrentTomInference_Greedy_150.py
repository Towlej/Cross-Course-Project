...
def split_shots(eq_shots_dict, train_size=150, seed=42):
...
def CurrentTomInferenceZoomAll(EQ_shots_dict, rng_number, All_I_tom, inner_points_bool, rVec, zVec, zoom_factor):
    ...
    while True:
        ...
        for i in range(60):
            ...
            for shotname in shot_keys:
                ...
                MI = MI_gain(
                    expanded_cov[inner_points_bool][:, inner_points_bool],
                    Post.prior_cov
                )
    ...
    output_file = "Results/zoom_all_summary.txt"

    with open(output_file, "w") as f:

        f.write("===== PER-SHOT RESULTS =====\n\n")

        for r in shot_results:
            f.write(f"Shot: {r['shot']}\n")
            f.write(f"Ip: {r['Ip']} ± {r['Ip_std']}\n")
            f.write(f"Zc: {r['Zc']} ± {r['Zc_std']}\n")
            f.write(f"Rc: {r['Rc']} ± {r['Rc_std']}\n")
            f.write("\n")

        f.write("\n===== GLOBAL STATISTICS (150 SHOTS) =====\n\n")

        f.write(f"Ip mean: {summary_stats['Ip_mean']}\n")
        f.write(f"Ip std: {summary_stats['Ip_std']}\n")
        f.write(f"Zc mean: {summary_stats['Zc_mean']}\n")
        f.write(f"Zc std: {summary_stats['Zc_std']}\n")
        f.write(f"Rc mean: {summary_stats['Rc_mean']}\n")
        f.write(f"Rc std: {summary_stats['Rc_std']}\n")
...
if __name__ == "__main__":
    ...
    if all_shots == True:
        ...
        EQ_shots_dict_train, EQ_shots_dict_test = split_shots(EQ_shots_dict_full, train_size=150, seed=42)
    else:
        EQ_shots_dict = Load_180_shots_dict()
